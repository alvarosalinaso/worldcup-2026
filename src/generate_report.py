...
def fig_to_div(fig):
    return fig.to_html(
        ...
        div_id=fig.layout.title.text.replace(" ", "_"),
    )
...
def build_comparison_overview():
    ...
    total_matches = list(hist["total_matches"]) + [
        int(data_2026["total_matches"].iloc[0])
    ]
...
def build_champions_timeline():
    hist = query(
        "SELECT year, champion, runner_up, top_scorer, top_scorer_goals FROM world_cups ORDER BY year",
        DB_HIST,
    )
...
def build_top_scorers():
    scorers = query(
        ...
        "FROM goals g JOIN teams t ON g.team_id = t.team_id WHERE g.own_goal = 0 "
        "GROUP BY g.scorer, t.name ORDER BY goals DESC LIMIT 10",
        DB_2026,
    )
