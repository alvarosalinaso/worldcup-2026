...
def validate_historical_data(db_path: str) -> dict[str, Any]:
    ...
    for row in wc:
        ...
        if row["total_goals"] < 100 or row["total_goals"] > 300:
            results["warnings"].append(
                f"Unusual goal count for {row['year']}: {row['total_goals']}"
            )
    ...
    for m in matches:
        if m["home_score"] < 0 or m["away_score"] < 0:
            results["errors"].append(
                f"Negative score: {m['home_team']} vs {m['away_team']}"
            )
            results["valid"] = False

        if m["home_score"] > 15 or m["away_score"] > 15:
            results["warnings"].append(
                f"Unusually high score: {m['home_team']} {m['home_score']}-{m['away_score']} {m['away_team']}"
            )

    # 3. Check goals reference valid matches
    goals = conn.execute(
        "SELECT wc_id, match_number, team_name, scorer, goals FROM goals"
    ).fetchall()
    ...
    for g in goals:
        ...
        if not match_exists:
            results["errors"].append(
                f"Goal references non-existent match: wc={g['wc_id']}, match={g['match_number']}"
            )
            results["valid"] = False

        if g["goals"] < 1 or g["goals"] > 5:
            results["warnings"].append(
                f"Unusual goal count for {g['scorer']}: {g['goals']}"
            )
    ...
    for s in standings:
        if s["teams"] != 4:
            results["warnings"].append(
                f"Group {s['group_letter']} WC{s['wc_id']} has {s['teams']} teams (expected 4)"
            )
    ...
    for st in standings_teams:
        ...
        if not team_exists:
            results["warnings"].append(
                f"Team {st['team_name']} in WC{st['wc_id']} not found in teams table"
            )
