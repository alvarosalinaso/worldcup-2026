...
# ── SIDEBAR ──
st.sidebar.image(
    "https://upload.wikimedia.org/wikipedia/en/thumb/6/6d/2026_FIFA_World_Cup_emblem.svg/240px-2026_FIFA_World_Cup_emblem.svg.png",
    width=120,
)
...
def load_kwargs():
    c = query(
        "SELECT DISTINCT round FROM matches ORDER BY CASE round WHEN 'Group Stage' THEN 1 WHEN 'Round of 32' THEN 2 WHEN 'Round of 16' THEN 3 WHEN 'Quarterfinal' THEN 4 WHEN 'Semifinal' THEN 5 WHEN 'Third Place' THEN 6 WHEN 'Final' THEN 7 END"
    )
...
def flag_url(team):
    _ = team.lower().replace(" ", "_")
    return "https://upload.wikimedia.org/wikipedia/en/thumb/9/9a/Flag_of_Spain.svg/40px-Flag_of_Spain.svg.png"
...
if section == "Tournament Overview":
    ...
    st.markdown(
        "*Data Analyst Portfolio Project | Data: Wikipedia | Stack: Python, SQLite, Streamlit, Plotly*"
    )
    st.markdown("---")

    col1, col2, col3, col4 = st.columns(4)
    df = query("SELECT COUNT(*) as n FROM teams")
    t_teams = df["n"][0]
    df = query("SELECT COUNT(*) as n FROM matches")
    t_matches = df["n"][0]
    df = query("SELECT SUM(home_score + away_score) as n FROM matches")
    t_goals = df["n"][0]
    df = query("SELECT SUM(attendance) as n FROM matches WHERE attendance IS NOT NULL")
    t_att = df["n"][0] or 6810966
    ...
    st.markdown("### Awards")
    awards_df = query(
        "SELECT award_name, recipient, t.name FROM awards a LEFT JOIN teams t ON a.team_id = t.team_id"
    )
    ...
    sm["lat"] = (
        ...
        .str[0]
        .str.strip()
        .map(lambda x: coords.get(x, (0, 0))[0])
    )
    sm["lon"] = (
        sm["name"]
        .str.split("(")
        .str[0]
        .str.strip()
        .map(lambda x: coords.get(x, (0, 0))[1])
    )
    fig = px.scatter_mapbox(
        ...
        size_max=30,
        zoom=2.2,
        title="16 Host Venues Across North America",
...
# ── GROUP STAGE ──
        ...
        with cols[idx % 3]:
            ...
            st.dataframe(
                df[
                    ...
                    lambda v: "background-color: #d4edda" if "(Q)" in str(v) else "",
                    subset=[" "],
                ),
                hide_index=True,
    ...
    df = query(
        "SELECT (m.home_score + m.away_score) AS goals FROM matches m WHERE m.round = 'Group Stage'"
    )
    fig = px.histogram(
        df,
        x="goals",
        nbins=12,
        title="Group Stage Goals per Match",
        labels={"goals": "Goals", "count": "Matches"},
...
# ── KNOCKOUT STAGE ──
    ...
    with tab1:
        ...
        round_matches = {
            r: bd[bd["round"] == r].reset_index(drop=True) for r in rounds_order
        }
        for ci, rnd in enumerate(rounds_order):
            with cols[min(ci, 5)]:
                ...
                for _, m in rm.iterrows():
                    ...
                    pen = (
                        f" ({int(m['home_penalty'])}-{int(m['away_penalty'])}p)"
                        if pd.notna(m["home_penalty"])
                        else ""
                    )
                    if m["home_score"] == m["away_score"] and pd.notna(
                        m["home_penalty"]
                    ):
                        winner = (
                            m["home"]
    ...
    with tab2:
        ...
        for _, row in df.iterrows():
            hp = (
                f" ({int(row['home_penalty'])}-{int(row['away_penalty'])} pens)"
            ...
            st.markdown(
                f"**{row['round']}** | {row['date']} | {row['home']} {score} {row['away']}"
            )
            st.caption(
                f" {row['stadium']} | Attendance: {row['attendance']:,}"
                if row["attendance"]
                else f" {row['stadium']}"
            )
            st.markdown("---")
    ...
    with tab3:
        ...
        fig = px.bar(
            ...
            color="Avg",
            text="Goals",
            title="Goals Scored per Stage",
            labels={"Goals": "Total Goals", "Avg": "Avg per Match"},
...
# ── TOP SCORERS ──
    ...
    st.markdown(
        "Mbappé won his second consecutive Golden Boot and became the all-time World Cup leading scorer (13 total)."
    )
...
# ── VENUE ANALYSIS ──
    ...
    tab1, tab2, tab3 = st.tabs(
        ["Attendance by Venue", "Utilization Rate", "Stadium Map"]
    )
    ...
    with tab1:
        fig = px.bar(
            att_df.sort_values("Total"),
            x="Total",
            y="Stadium",
            color="Pct_Full",
            text="Total",
            orientation="h",
            labels={"Total": "Total Attendance", "Stadium": ""},
    ...
    with tab2:
        fig = px.bar(
            ...
            color="Pct_Full",
            text="Pct_Full",
            labels={"Pct_Full": "Avg % Occupied"},
            color_continuous_scale="RdYlGn",
        )
        fig.update_traces(texttemplate="%{text}%", textposition="outside")
        fig.update_layout(xaxis_tickangle=-45)
        fig.add_hline(
            y=100, line_dash="dash", line_color="gray", annotation_text="Capacity"
        )
        st.plotly_chart(fig, use_container_width=True)
    ...
    with tab3:
        fig = px.scatter_mapbox(
            ...
            size_max=30,
            zoom=2.5,
            title="Stadium Locations & Match Count",
    ...
    with st.expander("Full Stadium Data"):
        st.dataframe(
            att_df[
                ["Stadium", "City", "Country", "Capacity", "Matches", "Total", "Avg"]
            ],
            hide_index=True,
            use_container_width=True,
        )
...
# ── CHAMPION PATH ──
    ...
    st.markdown(
        "Spain won its **2nd World Cup title** (first since 2010), becoming the first nation to hold both men's and women's World Cups simultaneously."
    )
...
# ── SURPRISES & DEBUTANTS ──
    ...
    with tab1:
        ...
        st.markdown(
            "Four teams made their World Cup debut in 2026: Cape Verde, Curaçao, Jordan, and Uzbekistan."
        )
        db = debutants()
        ...
        with col1:
            fig = px.bar(
                ...
                color="W",
                text="Pld",
                labels={"Pld": "Matches Played", "W": "Wins"},
                color_continuous_scale="Blues",
            )
            fig.update_traces(textposition="outside")
            st.plotly_chart(fig, use_container_width=True)
        ...
        st.info(
            " Cape Verde was the standout debutant: undefeated in the group stage (3 draws), advanced to the Round of 32, and pushed Argentina to extra time."
        )
    ...
    with tab2:
        ...
        fig = px.bar(
            ...
            color="GD",
            text="GD",
            title="Top 20 by Goal Difference",
            color_continuous_scale="RdYlGn",
        )
        fig.update_traces(textposition="outside")
        st.plotly_chart(fig, use_container_width=True)
    ...
    with tab3:
        ...
        fig = px.scatter(
            ...
            color="Confed",
            title="FIFA Rank vs Tournament Wins",
        )
        fig.update_traces(textposition="top center")
        st.plotly_chart(fig, use_container_width=True)
        ...
        st.markdown(
            "- **Canada** (Rank 30): Won 1 match, advanced past group stage as host"
        )
        st.markdown("- **DR Congo** (Rank 46): Advanced as best 3rd place from Group K")
        st.markdown(
            "- **South Africa** (Rank 60): Advanced as 2nd in Group A over South Korea"
        )
        st.markdown(
            "- All top 4 seeds (Spain, Argentina, France, England) reached semifinals — first time ever"
        )
