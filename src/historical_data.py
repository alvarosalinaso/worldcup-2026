...
def seed_2022(c, wc_id):
    ...
    for name, conf in teams:
        c.execute(
            "INSERT INTO teams (wc_id, name, confederation) VALUES (?,?,?)",
            (wc_id, name, conf),
        )
    ...
    for g_letter, teams_list in group_data.items():
        c.execute(
            "INSERT INTO groups (wc_id, group_letter) VALUES (?,?)", (wc_id, g_letter)
        )
        for pos, team, pld, w, d, loss, gf, ga, gd, pts, qual in teams_list:
            c.execute(
                """INSERT INTO group_standings
                (wc_id, group_letter, team_name, pos, played, wins, draws, losses,
                 goals_for, goals_against, goal_diff, points, qualified)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (wc_id, g_letter, team, pos, pld, w, d, loss, gf, ga, gd, pts, qual),
            )
