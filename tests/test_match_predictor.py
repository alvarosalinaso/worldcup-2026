...
def _setup_databases(tmp_path):
    ...
    """)
    wc_teams = [
        ("Argentina", "CONMEBOL", 1, 0),
        ("France", "UEFA", 2, 0),
        ("Brazil", "CONMEBOL", 3, 0),
        ("England", "UEFA", 4, 0),
        ("Belgium", "UEFA", 5, 0),
        ("Croatia", "UEFA", 6, 0),
        ("Netherlands", "UEFA", 7, 0),
        ("Portugal", "UEFA", 8, 0),
        ("Spain", "UEFA", 9, 0),
        ("Morocco", "CAF", 10, 0),
        ("Germany", "UEFA", 11, 0),
        ("Uruguay", "CONMEBOL", 12, 0),
        ("USA", "CONCACAF", 13, 0),
        ("Mexico", "CONCACAF", 14, 0),
        ("Japan", "AFC", 15, 0),
        ("Senegal", "CAF", 16, 0),
    ]
    for name, conf, rank, debut in wc_teams:
        c.execute(
            "INSERT INTO teams (name, confederation, fifa_ranking, debut) VALUES (?,?,?,?)",
            (name, conf, rank, debut),
        )
    conn.commit()
    conn.close()
...
def test_get_all_teams(tmp_path, monkeypatch):
    ...
    assert len(teams) == 16
    assert all(
        "name" in t and "confederation" in t and "fifa_ranking" in t for t in teams
    )
